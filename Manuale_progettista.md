...
Nella prima fase (`n_fase=1` come da inizializzazione) si attende l'esecuzione del comando iniziale, che prevede il tracciamento del rettangolo che deve coprire l'intero bordo schermo rimanendo all'interno di un range di accettabilità di 20 pixel concepito come cornice interna nella quale il comando è ancora valido. Una volta completato il comando iniziale il campo `n_fase` viene portato a 2 e in questa fase si attende un comando da tastiera che va a riempire il parametro `formato` che verra usato per il back-up. In questa fase buona parte dei tasti ('a'/'A' - 'm'/'M') è associata ad un comportamento specifico che va dall'operazione completa ('a'/'A'), che copia l'intero sottoalbero dalla directory sorgente a quella di destinazione, alla copia dei soli file con quell'estensione specificata. Una volta riempita questa variabile si passa alla terza fase (`n_fase=3`) in cui si attende il completamento del comando di conferma. Questo comando consiste nel tracciare una linea orizzontale, da un lato all'altro dello schermo, a qualsiasi altezza, rimanendo anche in questo caso all'interno di un range di accettabilità di 100 pixel. L'ultima fase invece, l'unica a non essere caratterizzata da un valore di `n_fase`, ma che viene avviata dopo la conferma, è la vera e propria operazione di back-up, con le relative finestre grafiche, al termine della quale si ritorna alla fase iniziale. Le dimensioni dello schermo in pixel, infine, sono gestite da due costanti i cui valori, però, non sono predefiniti, ma si adattano alla risoluzione del PC in uso.

## Avvio

Al momento dell'avvio del processo viene istanziato il relativo programma eseguibile, che viene inserito nella lista dei programmi che vengono eseguiti automaticamente all'avvio. Per fare questo si usa il crate `auto-launch` e la sua libreria interna `AutoLaunchBuilder`.
...
- L'applicazione generata potrebbe già essere settata come automatica quindi bisogna verificarlo con il metodo `is_enabled()`:
  - nel caso non si riuscisse ad ottenere tale informazione viene lanciato un errore;
...
- I campo `pos_prec_y` contiene la coordinata y alla quale è stato iniziato il comando di conferma, mediante il tracciamento della linea orizzontale. Vista la possibile inagibilità dello schermo, l'altezza a cui eseguire il comando è lasciata libera, essendo impossibile per l'utente identificare la posizione del mouse senza l'uso del monitor;
- I due campi `is_active` e `is_active_vec` vengono utilizzati per la corretta esecuzione dei comandi dall'esterno:
  - `is_active` monitora la corretta esecuzione del comando di conferma
  - `is_active_vec` è un array booleano di 4 elementi che rappresentano i 4 bordi dello schermo e indica se questi sono stati raggiunti correttamente oppure no;
- I due campi `range_rettangolo` e `range_conferma` sono pensati come range di accettabilità per i comandi con il mouse. `range_rettangolo` è pensato come margine di accettabilità (20 pixel) consentito durante il tracciamento del rettangolo lungo i bordi dello schermo; `range_conferma`, invece, è il range di accettabilità (100 pixel) per consentire all'utente di tracciare la linea orizzontale di conferma, essendo quest'ultimo comando più difficile da eseguire rispetto al precedente se non si ha riscontro visivo dallo schermo.
...
- `inizio_attesa` viene utilizzato quando, durante la fase 1, si effettua un click con il mouse. Se la posizione del cursore è nell'intorno dell'angolo in alto a sinistra, considerando `range_rettangolo`, tale angolo viene etichettato come "cliccato" nel vettore `is_active_vec`, impostando il valore a `true`.
- `fine_attesa`, invece, viene utilizzato sempre in fase 1, ma nel caso in cui viene rilasciato il mouse. Se di nuovo la posizione è nell'intorno dell'angolo in alto a sinistra, e questa volta `is_active_vec` contiene 4 valori a `true`, allora si passa alla fase 2.
- `cambia_posizione_per_rettangolo` è il metodo che gestisce il movimento del mouse nel caso in cui `n_fase` sia 1. Una volta ricevute dalla funzione `main` le nuove coordinate, si verifica che il cursore del mouse sia ancora all'interno del range valido per il comando. A seconda del vettore `is_active_vec`, si procede verso il prossimo angolo dello schermo che dovrebbe essere visitato durante il comando. Una volta trovato si verifica che la nuova posizione non ecceda i limiti previsti dal range; se così è l'intero vettore viene azzerato e l'operazione deve essere eseguita da capo; altrimenti, nel caso oin cui il prossimo angolo sia stato raggiunto correttamente, lo si segnala attraverso `is_active_vec`. Infine si sovrascrivono i campi `pos_x` e `pos_y` con i nuovi valori ricevuti;
- `attivazione_conferma` è il metodo duale di `inizio_attesa`, però utilizzato per il comando di conferma. Se il mouse, qualunque sia il valore di `pos_y`, si trova all'estremo sinistro, o all'interno del range, viene salvato il valore di `pos_y` nel campo `pos_prec_y` in modo da avere un riscontro alla fine del comando per verificare il corretto tracciamento del comando. Inoltre, viene impostato a `true` il booleano `is_active`, in modo da distinguere una corretta esecuzione da una che, nonostante abbia portato il cursore a destra, non ha rispettato il range, o sia stata interrotta rilasciando il mouse in anticipo;
- `disattivazione_conferma`, invece, si occupa del rilascio del mouse in fase di conferma. Anche in questo caso, se ci troviamo all'estremo destro, considerando il range, e il booleano `is_active` segna che l'operazione è ancora vaida si lancia direttamente la funzione di back-up, passando come parametro la variabile ricevuta dal metodo relativo al formato dei file di backup (vedi sezione "Gestione comando esterno");
...
- Attraverso la funzione `Command::new("cmd")` si istanzia un processo che agisce sul terminale;
- Con il metodo `args` si passano al processo figlio gli argomenti, ossia il codice per ottenere il nome e il refresh rate delle schede grafiche presenti sul dispositivo;
- Con il metodo `output` si attende la terminazione prelevando il risultato;

Ora nella variabile `p`, sono presenti i nomi e i relativi refresh rate delle schede grafiche del dispositivo. Per ottenere l'indice della scheda attiva:

- Si estrae il risultato attraverso `String::from_utf8_lossy(&p.stdout)`;
- Con il metodo `lines().skip(1).collect()` si trasforma il risultato in un vettore di stringhe;
- Si identifica l'indice della scheda grafica attiva tramite una ricerca all'interno del vettore utilizzando `chars().nth(0).unwrap().is_digit(10)` per verificare che il refresh rate sia un valore valido;
- `graph_index` viene impostata al valore della linea del vettore trovata incrementata di 1.

Successivamente si genera il secondo processo:
...
Ora nella variabile `program`, oltre ad altre informazioni, sono presenti le risoluzioni dei display delle schede grafiche. Per ottenerla quella desiderata:
...
- Si preleva solo la linea effettivamente utile con `nth(graph_index).unwrap().to_string()`;
...
Nota: il secondo valore ottenuto dopo la separazione contiene spazi, che devono essere successivamente eliminati prima della conversione con il metodo `trim()`.

## Gestione comando esterno

La gestione del comando dall'esterno rappresenta il passo più cruciale e delicato dell'intero programma. Una volta inizializzata la variabile che rappresenta il mouse, il thread principale, attraverso la funzione `listen`, viene messo in attesa di un evento dall'esterno. Questa attesa non è sbloccabile, a meno che non si interrompa lo stesso thread, cosa che permette l'esistenza di più fasi separate senza creare problemi una volta deciso come distinguerle (campo `n_fase`). La funzione `listen` accetta come parametro una calback (`callback_comando`) che, una volta ricevuto in ingresso l'evento stesso, passato automaticamente da `listen`, accede al campo `event_type` il quale, attraverso l'enumerazione `EventType`, contiene il tipo dell'evento (`ButtonPress`, `ButtonRelease`, `KeyPress`, ...). Ovviamente il comportamento in risposta all'evento deve essere diverso a seconda della fase, il che porta ad un controllo a monte del valore di `n_fase`.
...
- `n_fase=2`: il singolo evento accettato in questa fase è il `KeyPress` e il valore di `n_fase` viene immediatamente impostato a 3, qualunque sia il tasto premuto. Nel caso in cui il tasto premuto non è fra quelli validi `n_fase` viene reimpostato a 1. Una volta eseguito l'unwrap del codice del tasto si controlla, attraverso un match, se rispecchia uno dei casi accettati e, se così è, si riempie il parametro `formato` secondo la logica;
...
- chiamata alla funzione `suono_comando` passando in ingresso un intero;
- apertura della tupla `(stream, stream_handle)` attraverso `OutputStream::try_default().unwrap`;
- generazione della coda segnali da riprodurre con `Sink::try_new(&stream_handle).unwrap()` passando lo stream creato al punto 2;
- iterazione basata sul valore passato come parametro:
  ...
  - si attende per un tempo preimpostato così da permettere la separazione fra i segnali;
...
- Una volta ottenuto il nome del nodo come stringa attraverso il metodo `file_name` si ottiene il path sorgente concatenando la directory parametro con il valore appena ottenuto, in quanto sono trattate come due semplici stringhe, separandoli mediante "/". La stessa cosa viene fatta utilizzando la stringa della directory di destinazione, al fine di costruire il nuovo path per il nodo;
- A questo punto bisogna separare i due casi, ossia il caso in cui il nodo sia un file o una directory, basandosi sui metodi `file_type`, `is_file` e `is_dir`.
...
- Alla fine del ciclo `for` si ritornano le tre variabili citate (nel caso in cui si copi un formato specifico non si ritorna il numero delle cartelle poichè non considrate nel back-up). Dopodichè viene creato il file di riepilogo nella stessa directory di destinazione, utilizzando le informazioni appena ritornate dalla funzione di back-up.

## Finestre grafiche

La funzione di back-up genera anche una finestra grafica per segnalare l'effettivo inizio dell'operazione, oppure un errore di creazione o lettura di una directory:

- Nel primo caso, nella funzione `crea_finestra_successo`, una volta passate le due directory tra le quali operare, si utilizza la libreria `Druid` per creare una finestra contenente un messaggio, creato nella funzione stessa, che contenga le due directory e l'ora letta attraverso `Local`;
